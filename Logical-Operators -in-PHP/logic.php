<?php

$rofiqul = 100;
$robiul = 200;
if ($rofiqul == $robiul){
    echo "rofiqul has same amount of money robiul";
}
elseif($rofiqul>$robiul){
    echo "rofuqul has more money than robiul";
}
elseif($rofiqul<$robiul){
    echo "rofuqul has less money than robiul";
}

echo "<br>";

$robimark = 68;
$rahatmark = 76;
$rahattmark = 76;
if ($robimark == $rahatmark){
    echo "your Result Is : A Grade";
}
elseif($robimark>$rahatmark){
    echo "your Result Is : A Grade";
}
elseif($robimark<$rahatmark){
    echo "your Result Is : A Grade";
}
echo "<br>";

$mark = 80;
if($mark>=80){
    $grade = "A+";
}
elseif ($mark >=70) {
    $grade = "A";
}
elseif($mark >= 60){
    $grade = "A-";
}
elseif($mark >= 50){
    $grade = "A-";
}
elseif($mark >= 45){
    $grade = "C-";
}
elseif($mark >= 40){
    $grade = "D-";
}
elseif($mark >= 33){
    $grade = "D-";
}
elseif($mark >= 30){
    $grade = "F-";
}

echo "<h2>Your Result Is: $grade</h2>";
echo "<h3>Congratulation you are passed!</h3>";

$mark = 65;

if($mark >=80){
    $grade = "A+";
}
elseif($mark >= 75){
    $grade = "A";
}

elseif($mark >= 65){
    $grade ="A-";
}

elseif ($mark >= 45){
    $grade = "D";
}
elseif($mark >= 35){
    $grade = "D";
}

elseif($mark >= 30){
    $grade = "F";
}

echo "Your Result Is :" .$grade;

echo "<br>";



$abdullah = 400;
$shipon = 500;
$rohim =1000;

if ($abdullah==$shipon){
    echo "someone money equal";
}
elseif($abdullah>$shipon){
    echo "shipon er theke abdullah tk besi ";
}
elseif($shipon<$rohim){
    echo "shipon er theke rohim er tk besi ";
}
elseif($abdullah>$shipon){
    echo "abdullah theke shipon er tk besi ";
}



